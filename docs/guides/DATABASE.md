...
`python3 tools/generate_database_sql.py`; the generator derives the latest schema version automatically. Never edit a migration
already applied to users' databases.

V10 labels existing ledger rows as SUPER_ACE and adds indexes for Dragon Tiger history. Wallets, passwords and earlier Flyway migrations are unchanged. Historical upgrade files remain for earlier releases; use a file ending in _to_v19.sql for this release.
...
V13 replaces those unique round keys with non-unique lookup indexes so multiple bets may share one table round. New bets reserve stakes until closing, then settle exactly once. Existing rows default to settled, preserving all prior payouts. Use `upgrade_v12_to_v19.sql` only when upgrading manually from V12; otherwise start the server and let Flyway apply the migration.

V19 adds private Lucky 9 decks and active-hand flags. Existing games and wallets are preserved.
