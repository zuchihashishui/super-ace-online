...
import philip.emerald.ace.crash.CrashEngine;
...
class ServerTest {
 ...
 @Test void saklaReceiptSelectionAndWalletIsolation(){var u=tree().player();var request=id();var r=arcade.play(u,"LOBBY",ArcadeEngine.Game.SAKLA,request,500,0,19);assertEquals(19,r.selection());assertEquals(999500+r.payoutCents(),lobby.wallet(u.id(),false).balanceCents());assertEquals(1000000,game.wallet(u.id(),false).balanceCents());assertEquals(r,arcade.play(u,"LOBBY",ArcadeEngine.Game.SAKLA,request,500,0,19));assertThrows(GameService.ApiError.class,()->arcade.play(u,"LOBBY",ArcadeEngine.Game.SAKLA,request,500,0,18));assertThrows(GameService.ApiError.class,()->arcade.play(u,"LOBBY",ArcadeEngine.Game.SAKLA,id(),500,1,20));assertEquals(r,arcade.history(u,"LOBBY",ArcadeEngine.Game.SAKLA).getFirst());}
 @Autowired philip.emerald.ace.luckynine.LuckyNineService ln;
 philip.emerald.ace.luckynine.LuckyNineService.Round activeNine(Accounts.User u){for(int i=0;i<80;i++){var r=ln.start(u,"LOBBY",id(),500,lobby.wallet(u.id(),false).revision());if(r.status().equals("ACTIVE"))return r;}throw new AssertionError("no active hand");}
 @Test void luckyNineSecretDecisionAndDuplicateCashCredit()throws Exception{var u=tree().player();var other=tree().player();var r=activeNine(u);assertNull(r.bankerCards());assertNull(r.bankerTotal());assertEquals(r,ln.start(u,"LOBBY",r.requestId(),500,0));assertThrows(GameService.ApiError.class,()->ln.start(u,"LOBBY",id(),500,r.revision()));assertThrows(GameService.ApiError.class,()->ln.start(u,"LOBBY",r.requestId(),600,0));assertThrows(GameService.ApiError.class,()->ln.move(other,"LOBBY",r.requestId(),philip.emerald.ace.luckynine.LuckyNineEngine.Action.STAND));
 db.update("UPDATE lucky_nine_deals SET deck_json='[2,0,3,1,13,14]' WHERE player_id=? AND request_id=?",u.id(),r.requestId());long before=lobby.wallet(u.id(),false).balanceCents();try(var pool=Executors.newFixedThreadPool(4)){var calls=new ArrayList<Future<philip.emerald.ace.luckynine.LuckyNineService.Round>>();for(int i=0;i<8;i++)calls.add(pool.submit(()->ln.move(u,"LOBBY",r.requestId(),philip.emerald.ace.luckynine.LuckyNineEngine.Action.STAND)));var first=calls.getFirst().get();for(var f:calls)assertEquals(first,f.get());assertEquals("WIN",first.status());assertEquals(975,first.payoutCents());assertEquals(List.of(0,1,13),first.bankerCards());assertEquals(first,ln.history(u,"LOBBY").getFirst());}assertEquals(before+975,lobby.wallet(u.id(),false).balanceCents());assertEquals(1000000,game.wallet(u.id(),false).balanceCents());assertThrows(GameService.ApiError.class,()->ln.move(u,"LOBBY",r.requestId(),philip.emerald.ace.luckynine.LuckyNineEngine.Action.HIT));mvc.perform(get("/api/lucky-nine/rounds")).andExpect(status().isUnauthorized());mvc.perform(post("/api/lucky-nine/start").header("X-Game-Client","web").contentType(MediaType.APPLICATION_JSON).content("{\"requestId\":\""+id()+"\",\"betCents\":500,\"expectedRevision\":0}")).andExpect(status().isUnauthorized());}
 @Test void luckyNineHitLossTieAndModeIsolation(){var u=tree().player();var r=activeNine(u);db.update("UPDATE lucky_nine_deals SET deck_json='[0,1,2,3,4,5]' WHERE player_id=? AND request_id=?",u.id(),r.requestId());var hit=ln.move(u,"LOBBY",r.requestId(),philip.emerald.ace.luckynine.LuckyNineEngine.Action.HIT);assertEquals(3,hit.playerCards().size());assertEquals("WIN",hit.status());r=activeNine(u);db.update("UPDATE lucky_nine_deals SET deck_json='[0,13,2,15,9,10]' WHERE player_id=? AND request_id=?",u.id(),r.requestId());var tie=ln.move(u,"LOBBY",r.requestId(),philip.emerald.ace.luckynine.LuckyNineEngine.Action.STAND);assertEquals("TIE",tie.status());assertEquals(500,tie.payoutCents());r=activeNine(u);db.update("UPDATE lucky_nine_deals SET deck_json='[0,1,2,3,4,5]' WHERE player_id=? AND request_id=?",u.id(),r.requestId());var lost=ln.move(u,"LOBBY",r.requestId(),philip.emerald.ace.luckynine.LuckyNineEngine.Action.STAND);assertEquals("LOST",lost.status());assertEquals(0,lost.payoutCents());assertTrue(ln.history(u,"CLUB").isEmpty());}
 @Test void bingoReceiptIdempotencyReportsAndHistory(){var u=tree().player();var request=id();var r=arcade.play(u,"LOBBY",ArcadeEngine.Game.BINGO,request,501,0);assertEquals(55,r.outcome().values().size());assertEquals(r.outcome().slot()==0?0:3381,r.payoutCents());assertEquals(999499+r.payoutCents(),lobby.wallet(u.id(),false).balanceCents());assertEquals(r,arcade.play(u,"LOBBY",ArcadeEngine.Game.BINGO,request,501,0));assertThrows(GameService.ApiError.class,()->arcade.play(u,"LOBBY",ArcadeEngine.Game.SAKLA,request,501,0));assertThrows(GameService.ApiError.class,()->arcade.play(u,"LOBBY",ArcadeEngine.Game.BINGO,id(),500,r.revision(),1));assertEquals(1000000,game.wallet(u.id(),false).balanceCents());for(int i=0;i<22;i++)arcade.play(u,"LOBBY",ArcadeEngine.Game.BINGO,id(),500,lobby.wallet(u.id(),false).revision());assertEquals(20,arcade.history(u,"LOBBY",ArcadeEngine.Game.BINGO).size());assertTrue(arcade.history(u,"CLUB",ArcadeEngine.Game.BINGO).isEmpty());}
 @Autowired MinesService mines;
 @Test void minesHidesBoardRestoresStateAndCashoutIsIdempotent()throws Exception{var u=tree().player();var other=tree().player();var r=mines.start(u,"LOBBY",id(),500,3,0);assertNull(r.mines());assertEquals(r,mines.start(u,"LOBBY",r.requestId(),500,3,0));assertEquals(999500,lobby.wallet(u.id(),false).balanceCents());assertThrows(GameService.ApiError.class,()->mines.start(u,"LOBBY",id(),500,3,1));assertThrows(GameService.ApiError.class,()->mines.move(u,"LOBBY",r.requestId(),null));assertThrows(GameService.ApiError.class,()->mines.move(other,"LOBBY",r.requestId(),3));
  db.update("UPDATE mines_boards SET mines_json='[0,1,2]' WHERE player_id=? AND request_id=?",u.id(),r.requestId());try(var pool=Executors.newFixedThreadPool(4)){var calls=new ArrayList<Future<MinesService.Round>>();for(int i=0;i<8;i++)calls.add(pool.submit(()->mines.move(u,"LOBBY",r.requestId(),3)));var first=calls.getFirst().get();for(var f:calls)assertEquals(first,f.get());assertNull(first.mines());assertEquals(List.of(3),first.opened());assertEquals(first,mines.history(u,"LOBBY").getFirst());}
  var paid=mines.move(u,"LOBBY",r.requestId(),null);assertEquals(550,paid.payoutCents());assertEquals(1000050,lobby.wallet(u.id(),false).balanceCents());assertEquals(List.of(0,1,2),paid.mines());assertEquals(paid,mines.move(u,"LOBBY",r.requestId(),null));assertEquals(paid,mines.move(u,"LOBBY",r.requestId(),4));assertEquals(1000000,game.wallet(u.id(),false).balanceCents());mvc.perform(get("/api/mines/rounds")).andExpect(status().isUnauthorized());}
 @Test void minesLossAndAllSafeCompletionSettleOnce(){var u=tree().player();var r=mines.start(u,"LOBBY",id(),500,1,0);db.update("UPDATE mines_boards SET mines_json='[0]' WHERE player_id=? AND request_id=?",u.id(),r.requestId());var lost=mines.move(u,"LOBBY",r.requestId(),0);assertEquals("LOST",lost.status());assertEquals(0,lost.payoutCents());assertEquals(lost,mines.move(u,"LOBBY",r.requestId(),null));var next=mines.start(u,"LOBBY",id(),500,1,lost.revision());db.update("UPDATE mines_boards SET mines_json='[0]' WHERE player_id=? AND request_id=?",u.id(),next.requestId());for(int i=1;i<25;i++)next=mines.move(u,"LOBBY",next.requestId(),i);assertEquals("CASHED_OUT",next.status());assertEquals(12125,next.payoutCents());assertEquals(1011125,lobby.wallet(u.id(),false).balanceCents());}
 @Test void minesInvalidInputsAndGameReplayAreRejected(){var u=tree().player();assertThrows(GameService.ApiError.class,()->mines.start(u,"LOBBY",id(),500,2,0));assertThrows(GameService.ApiError.class,()->mines.start(u,"BAD",id(),500,3,0));var r=mines.start(u,"LOBBY",id(),500,3,0);assertThrows(GameService.ApiError.class,()->mines.start(u,"LOBBY",r.requestId(),1000,3,1));assertThrows(GameService.ApiError.class,()->mines.move(u,"LOBBY",r.requestId(),25));assertThrows(GameService.ApiError.class,()->arcade.play(u,"LOBBY",ArcadeEngine.Game.SLOTS,r.requestId(),500,1));assertTrue(mines.history(u,"CLUB").isEmpty());}
 @Autowired ArcadeService arcade;
 @Test void slotsPreservesReceiptAndOwnHistory(){var u=tree().player();var r=arcade.play(u,"LOBBY",ArcadeEngine.Game.SLOTS,id(),500,0);assertEquals(r,arcade.play(u,"LOBBY",ArcadeEngine.Game.SLOTS,r.requestId(),500,0));assertEquals(1000000-500+r.payoutCents(),lobby.wallet(u.id(),false).balanceCents());assertTrue(arcade.history(u,"LOBBY",ArcadeEngine.Game.WHEEL).isEmpty());assertEquals(1,arcade.history(u,"LOBBY",ArcadeEngine.Game.SLOTS).size());}
 @Test void wheelUsesOwnLedgerAndRejectsCrossGameReplay(){var u=tree().player();var r=arcade.play(u,"LOBBY",ArcadeEngine.Game.WHEEL,id(),500,0);assertEquals(ArcadeEngine.WHEEL.get(r.outcome().slot()).intValue(),r.outcome().multiplierBps());assertEquals(1,arcade.history(u,"LOBBY",ArcadeEngine.Game.WHEEL).size());assertTrue(arcade.history(u,"LOBBY",ArcadeEngine.Game.PLINKO).isEmpty());assertThrows(GameService.ApiError.class,()->arcade.play(u,"LOBBY",ArcadeEngine.Game.PLINKO,r.requestId(),500,r.revision()));}
 @Test void plinkoConcurrentRetryIsAtomicAndIsolated()throws Exception{var u=tree().player();String request=id();long before=lobby.wallet(u.id(),false).balanceCents(),club=game.wallet(u.id(),false).balanceCents();try(var pool=Executors.newFixedThreadPool(4)){var calls=new ArrayList<Future<ArcadeService.Round>>();for(int i=0;i<8;i++)calls.add(pool.submit(()->arcade.play(u,"LOBBY",ArcadeEngine.Game.PLINKO,request,500,0)));var first=calls.getFirst().get();for(var call:calls)assertEquals(first,call.get());assertEquals(before-500+first.payoutCents(),lobby.wallet(u.id(),false).balanceCents());}assertEquals(club,game.wallet(u.id(),false).balanceCents());assertEquals(1,arcade.history(u,"LOBBY",ArcadeEngine.Game.PLINKO).size());assertThrows(GameService.ApiError.class,()->arcade.play(u,"LOBBY",ArcadeEngine.Game.PLINKO,request,1000,1));assertThrows(GameService.ApiError.class,()->arcade.play(u,"LOBBY",ArcadeEngine.Game.PLINKO,id(),500,0));mvc.perform(get("/api/arcade/PLINKO/rounds")).andExpect(status().isUnauthorized());mvc.perform(get("/api/arcade/plinko/rules")).andExpect(status().isOk()).andExpect(jsonPath("$.rows").value(12));}
 @Autowired CrashService crash;
 long fixedCrash(int point){var table=crash.table("LOBBY");long flight=System.currentTimeMillis()+10000,at=CrashEngine.at(flight,point);db.update("UPDATE crash_rounds SET flight_at=?,crash_at=?,next_at=?,crash_bps=? WHERE round_id=?",flight,at,at+5000,point,table.roundId());return table.roundId();}
 @Test void crashAutoSettlementRetryAndWalletIsolation()throws Exception{
  var u=tree().player();long round=fixedCrash(500);long before=lobby.wallet(u.id(),false).balanceCents(),club=game.wallet(u.id(),false).balanceCents();
  var b=crash.bet(u,"LOBBY",round,id(),500,200,0);assertEquals(before-500,lobby.wallet(u.id(),false).balanceCents());assertEquals(b,crash.bet(u,"LOBBY",round,b.requestId(),500,200,0));
  assertThrows(GameService.ApiError.class,()->crash.bet(u,"LOBBY",round,b.requestId(),500,300,1));assertThrows(GameService.ApiError.class,()->crash.bet(u,"LOBBY",round,id(),500,200,1));
  assertNull(crash.table("LOBBY").crashBps());assertNull(crash.table("LOBBY").nextRoundAt());var secret=crash.round(round);long due=CrashEngine.at(secret.flight(),200);
  crash.settle("LOBBY",u.id(),due-1);assertEquals(before-500,lobby.wallet(u.id(),false).balanceCents());crash.settle("LOBBY",u.id(),due);crash.settle("LOBBY",u.id(),secret.crash()+1);
  assertEquals(before+500,lobby.wallet(u.id(),false).balanceCents());assertEquals(club,game.wallet(u.id(),false).balanceCents());assertEquals("CASHED_OUT",crash.bets(u,"LOBBY").getFirst().status());assertEquals(1000,crash.cashout(u,"LOBBY",b.requestId()).payoutCents());
  mvc.perform(get("/api/crash/table")).andExpect(status().isOk()).andExpect(jsonPath("$.crashBps").isEmpty());mvc.perform(get("/api/crash/bets")).andExpect(status().isUnauthorized());
 }
 @Test void crashTieLosesAndManualCashoutPaysOnce()throws Exception{
  var u=tree().player();var other=tree().player();long round=fixedCrash(200);var a=crash.bet(u,"LOBBY",round,id(),500,200,0);var b=crash.bet(other,"LOBBY",round,id(),500,0,0);
  assertThrows(GameService.ApiError.class,()->crash.cashout(other,"LOBBY",b.requestId()));assertThrows(GameService.ApiError.class,()->crash.cashout(u,"LOBBY",b.requestId()));
  crash.settle("LOBBY",u.id(),crash.round(round).crash());assertEquals("LOST",crash.bets(u,"LOBBY").getFirst().status());assertEquals(999500,lobby.wallet(u.id(),false).balanceCents());
  long flight=System.currentTimeMillis()-2000;db.update("UPDATE crash_rounds SET flight_at=?,crash_at=?,next_at=? WHERE round_id=?",flight,flight+100000,flight+105000,round);
  long before=lobby.wallet(other.id(),false).balanceCents();try(var pool=Executors.newFixedThreadPool(4)){var calls=new ArrayList<Future<CrashService.Bet>>();for(int i=0;i<6;i++)calls.add(pool.submit(()->crash.cashout(other,"LOBBY",b.requestId())));var first=calls.getFirst().get();assertTrue(first.payoutCents()>=640);for(var call:calls)assertEquals(first,call.get());assertEquals(before+first.payoutCents(),lobby.wallet(other.id(),false).balanceCents());}
 }
 @Test void crashRejectsInvalidStaleAndCrossGameRequests(){var u=tree().player();long round=fixedCrash(100);assertThrows(GameService.ApiError.class,()->crash.bet(u,"LOBBY",round,id(),499,200,0));assertThrows(GameService.ApiError.class,()->crash.bet(u,"LOBBY",round,id(),500,100,0));assertThrows(GameService.ApiError.class,()->crash.bet(u,"LOBBY",round,id(),500,200,2));
  var b=crash.bet(u,"LOBBY",round,id(),500,0,0);crash.settle("LOBBY",u.id(),crash.round(round).crash());assertEquals("LOST",crash.bets(u,"LOBBY").getFirst().status());assertThrows(GameService.ApiError.class,()->colorGame.play(u,"LOBBY",1,b.requestId(),ColorGameEngine.Side.RED,500,2));assertThrows(GameService.ApiError.class,()->crash.bet(u,"BAD",round,id(),500,200,0));
 }
 ...
 @Test void colorExampleReturnsThirtyAndThreeColorsAreEnforced(){var u=tree().player();long round=bettingRound();long before=lobby.wallet(u.id(),false).balanceCents();
  var red=colorGame.play(u,"LOBBY",round,id(),ColorGameEngine.Side.RED,1000,0);
  colorGame.play(u,"LOBBY",round,id(),ColorGameEngine.Side.YELLOW,1000,1);
  colorGame.play(u,"LOBBY",round,id(),ColorGameEngine.Side.WHITE,1000,2);
  var ex=assertThrows(GameService.ApiError.class,()->colorGame.play(u,"LOBBY",round,id(),ColorGameEngine.Side.BLUE,1000,3));assertEquals("COLOR_LIMIT",ex.code);assertEquals(before-3000,lobby.wallet(u.id(),false).balanceCents());
  assertEquals(red,colorGame.play(u,"LOBBY",round,red.requestId(),ColorGameEngine.Side.RED,1000,0));
  var outcome=new ColorGameEngine.Outcome(List.of(ColorGameEngine.Side.GREEN,ColorGameEngine.Side.RED,ColorGameEngine.Side.RED));
  db.update("UPDATE color_game_rounds SET outcome_json=? WHERE round_id=?",game.encode(outcome),round);
  colorGame.settle("LOBBY",u.id(),red.revealAt());var rows=colorGame.historyAt(u,"LOBBY",red.revealAt());assertEquals(3000,rows.stream().mapToLong(ColorGameService.Round::payoutCents).sum());assertEquals(before,lobby.wallet(u.id(),false).balanceCents());
  colorGame.settle("LOBBY",u.id(),red.revealAt());assertEquals(before,lobby.wallet(u.id(),false).balanceCents());
 }
 @Test void colorLimitAllowsTopUpsAndIsScopedByPlayerWalletRound(){var u=tree().player();var v=tree().player();long round=bettingRound();for(int i=0;i<3;i++)colorGame.play(u,"LOBBY",round,id(),ColorGameEngine.Side.values()[i],500,i);colorGame.play(u,"LOBBY",round,id(),ColorGameEngine.Side.YELLOW,500,3);assertEquals(998000,lobby.wallet(u.id(),false).balanceCents());colorGame.play(u,"CLUB",round,id(),ColorGameEngine.Side.GREEN,500,0);colorGame.play(v,"LOBBY",round,id(),ColorGameEngine.Side.GREEN,500,0);assertEquals(999500,game.wallet(u.id(),false).balanceCents());}
