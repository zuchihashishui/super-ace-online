...
class MigrationV16Test {
 @Test void colorGameMigrationPreservesExistingWallets(){
  var source=new DriverManagerDataSource("jdbc:h2:mem:migration_v16;MODE=MySQL;DATABASE_TO_LOWER=TRUE;DB_CLOSE_DELAY=-1","sa","");
  Flyway.configure().dataSource(source).locations("classpath:db/migration").target("15").load().migrate();var db=new JdbcTemplate(source);
  ...
  assertEquals(0,db.queryForObject("SELECT COUNT(*) FROM color_game_rounds",Integer.class));
 }
}
