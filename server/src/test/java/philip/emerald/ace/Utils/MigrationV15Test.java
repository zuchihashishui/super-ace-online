...
class MigrationV15Test {
 @Test void dailyRewardMigrationPreservesExistingWallets(){
  var source=new DriverManagerDataSource("jdbc:h2:mem:migration_v15;MODE=MySQL;DATABASE_TO_LOWER=TRUE;DB_CLOSE_DELAY=-1","sa","");
  Flyway.configure().dataSource(source).locations("classpath:db/migration").target("14").load().migrate();var db=new JdbcTemplate(source);
  ...
  assertEquals(0,db.queryForObject("SELECT COUNT(*) FROM lobby_daily_rewards",Integer.class));
 }
}
