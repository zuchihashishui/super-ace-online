...
public class ColorGameService {
 ...
 public record Table(long roundId,String phase,long serverTime,long bettingClosesAt,long nextRoundAt,int secondsRemaining,ColorGameEngine.Outcome outcome){}
 public record TableResult(long tableRoundId,long createdAt,ColorGameEngine.Outcome outcome){}
 public List<TableResult> tableHistory(){return club.db().query("SELECT round_id,betting_closes_at,outcome_json FROM color_game_rounds WHERE betting_closes_at<=? ORDER BY round_id DESC LIMIT 20",(r,n)->new TableResult(r.getLong(1),r.getLong(2),club.decode(r.getString(3),ColorGameEngine.Outcome.class)),now());}
 public record Round(String requestId,String mode,long tableRoundId,long revealAt,ColorGameEngine.Side side,long betCents,long payoutCents,long balanceBeforeCents,long balanceCents,long revision,long createdAt,ColorGameEngine.Outcome outcome){}
 public record SidePool(ColorGameEngine.Side side,long ownCents,long ownChipCount,long othersCents,long othersChipCount,int otherBettors){}
 ...
 public Crowd crowd(Accounts.User viewer,String mode,long id){
  ...
  var sums=new EnumMap<ColorGameEngine.Side,long[]>(ColorGameEngine.Side.class);
  var others=new EnumMap<ColorGameEngine.Side,Set<String>>(ColorGameEngine.Side.class);var bettors=new HashSet<String>();
  for(var side:ColorGameEngine.Side.values()){sums.put(side,new long[4]);others.put(side,new HashSet<>());}
  // Only aggregate confirmed stakes. No names, account IDs, cards or payouts leave this method.
  game.db().query("SELECT player_id,response_json FROM "+game.table("round_ledger")+" WHERE game_type='COLOR_GAME' AND game_round_id=?",(org.springframework.jdbc.core.RowCallbackHandler)rs->{
  ...
  var sides=new ArrayList<SidePool>();for(var side:ColorGameEngine.Side.values()){var s=sums.get(side);sides.add(new SidePool(side,s[0],s[1],s[2],s[3],others.get(side).size()));}
 ...
 public ColorGameEngine.Outcome shared(long id){
  var rows=club.db().queryForList("SELECT outcome_json FROM color_game_rounds WHERE round_id=?",String.class,id);
  if(!rows.isEmpty())return club.decode(rows.getFirst(),ColorGameEngine.Outcome.class);
  var outcome=engine.deal();
  try{club.db().update("INSERT INTO color_game_rounds(round_id,starts_at,betting_closes_at,reveal_ends_at,outcome_json) VALUES(?,?,?,?,?)",id,startsAt(id),startsAt(id)+BETTING_MS,startsAt(id)+CYCLE_MS,club.encode(outcome));return outcome;}
  catch(DuplicateKeyException ignored){return club.decode(club.db().queryForObject("SELECT outcome_json FROM color_game_rounds WHERE round_id=? FOR UPDATE",String.class,id),ColorGameEngine.Outcome.class);}
 }
 ...
 @Transactional public Round play(Accounts.User actor,String mode,long tableRoundId,String requestId,ColorGameEngine.Side side,long bet,long revision){
  ...
  // The wallet lock serializes concurrent tabs. Count confirmed colours for this player,
  // this wallet and this round; idempotent replays were handled before this check.
  var used=new HashSet<ColorGameEngine.Side>(game.db().query("SELECT response_json FROM "+game.table("round_ledger")+" WHERE player_id=? AND game_type='COLOR_GAME' AND game_round_id=?",(r,n)->game.decode(r.getString(1),Round.class).side(),u.id(),current));
  if(!used.contains(side)&&used.size()>=3)throw GameService.error(409,"COLOR_LIMIT");
 ...
 @Transactional public void settle(String mode,String playerId,long time){
  ...
  var rows=game.db().query("SELECT response_json FROM "+game.table("round_ledger")+" WHERE player_id=? AND game_type='COLOR_GAME' AND cg_settled=FALSE AND game_round_id<=? ORDER BY wallet_revision",(r,n)->game.decode(r.getString(1),Round.class),playerId,Math.floorDiv(time-BETTING_MS,CYCLE_MS));
  long balance=wallet.balanceCents(),revision=wallet.revision();
  for(var row:rows){
   var outcome=shared(row.tableRoundId());long payout=ColorGameEngine.payout(row.betCents(),row.side(),outcome);balance=Math.addExact(balance,payout);revision++;
   ...
   game.db().update("UPDATE "+game.table("round_ledger")+" SET cg_settled=TRUE,payout_cents=?,response_json=? WHERE player_id=? AND request_id=?",payout,game.encode(settled),playerId,row.requestId());
  }
 ...
 @Transactional public List<Round> historyAt(Accounts.User actor,String mode,long time){
  ...
  return game.db().query("SELECT response_json FROM "+ledger+" WHERE player_id=? AND game_type='COLOR_GAME' AND cg_settled=TRUE AND game_round_id IN (SELECT game_round_id FROM (SELECT DISTINCT game_round_id FROM "+ledger+" WHERE player_id=? AND game_type='COLOR_GAME' AND cg_settled=TRUE AND game_round_id<=? ORDER BY game_round_id DESC LIMIT 20) recent_games) ORDER BY game_round_id DESC,wallet_revision DESC",(r,n)->game.decode(r.getString(1),Round.class),actor.id(),actor.id(),Math.floorDiv(time-BETTING_MS,CYCLE_MS));
 }
}
