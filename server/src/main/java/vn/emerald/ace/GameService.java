...
public class GameService {
 ...
 SpinResult executeLocked(Accounts.User u,Wallet w,String requestId,long bet,String runId){
  ...
  var agent=u.role()==Accounts.Role.AGENT?u:(u.parentId()==null?null:accounts.user(u.parentId()));db.update("INSERT INTO "+table("round_ledger")+"(player_id,request_id,agent_id,super_agent_id,nominal_bet,wager_cents,payout_cents,is_free,wallet_revision,run_id,response_json,created_at,rtp_profile) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)",u.id(),requestId,agent==null?null:agent.id(),agent==null?null:agent.parentId(),bet,free?0:bet,outcome.winCents(),free,result.revision(),runId,encode(result),now,profile);return result;
 }
