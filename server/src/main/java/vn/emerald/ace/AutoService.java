...
public class AutoService {
 ...
 @Transactional public Job stop(Accounts.User u,String runId){accounts.require(u,Accounts.Role.PLAYER);game.wallet(u.id(),true);game.db().update("UPDATE "+game.table("auto_jobs")+" SET active=FALSE,reason='STOPPED' WHERE player_id=? AND run_id=?",u.id(),runId);return job(u.id());}
 @Transactional public void process(String id){var w=game.wallet(id,true);var j=job(id);long now=System.currentTimeMillis();if(j==null||!j.active()||j.nextAt()>now)return;var u=accounts.user(id);String reason=null;
  if(!u.enabled()||u.role()!=Accounts.Role.PLAYER)reason="ACCOUNT_DISABLED";else if(j.completed()>=j.planned()&&w.freeSpins()==0)reason="COMPLETED";else if(w.freeSpins()==0&&w.balanceCents()<j.betCents())reason="INSUFFICIENT_FUNDS";
