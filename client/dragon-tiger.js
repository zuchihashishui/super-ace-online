...
Object.assign(english,{
 dtRulesCards:'A fresh 52-card deck is used each round. Dragon and Tiger receive two cards each, without replacement.',
 dtRulesScore:'A = 1; 2–9 = face value; 10, J, Q, K = 0. Add both cards and keep the last digit. Higher score wins; equal scores are a Tie.',
 dtRulesPay:'Select a chip (5, 10, 15 or 20), then tap Dragon, Tiger or Tie. Each tap adds another bet. You may bet on several sides before the 10-second timer closes. Accepted bets cannot be cancelled. Dragon / Tiger return 1.95× on a win; Tie returns 9×. A tie refunds Dragon / Tiger stakes. Returns include the stake and are rounded to 0.01 for each bet.',
 dtRulesSeparate:'Independent two-card variant, not standard Dragon Tiger or full Baccarat. Fixed payouts; Super Ace RTP settings do not apply. Virtual chips only.',
...
Object.assign(filipino,{
 ...
 dtRulesCards:'Bagong 52 baraha bawat laro. Tig-dalawang baraha ang Dragon at Tiger, walang ibinabalik sa deck.',
 ...
 dtRulesPay:'Pumili ng chip (5, 10, 15 o 20) at pindutin ang Dragon, Tiger o Tabla. Bawat pindot ay dagdag na taya. Maaaring tumaya sa maraming panig sa loob ng 10 segundo. Hindi makakansela ang tinanggap na taya. Balik: Dragon / Tiger 1.95×; Tabla 9×. Kapag tabla, ibinabalik ang taya sa Dragon / Tiger. Kasama ang puhunan sa balik; niro-round sa 0.01 bawat taya.',
 dtRulesSeparate:'Hiwalay na dalawang-barahang bersiyon, hindi karaniwang Dragon Tiger o buong Baccarat. Hindi naaangkop ang RTP ng Super Ace. Virtual chips lamang.',
...
function dtClear(){for(const side of ['Dragon','Tiger']){$('dt'+side+'Cards').replaceChildren(dtCard(),dtCard());$('dt'+side+'Score').textContent='—';$('dt'+side+'Hand').classList.remove('winner')}$('dtResult').classList.remove('won');$('dtResultTitle').textContent=t('Choose your side');$('dtResultDetail').textContent=t('Higher score wins · 0–9 points');document.querySelectorAll('[data-dt-side]').forEach(b=>b.classList.remove('winning'))}
function dtClock(){
