...
function cgReveal(outcome){
 ...
 if(!reduced){$('cgDice').querySelectorAll('.cg-die').forEach((die,i)=>{const cube=die.querySelector('.cg-cube'),dir=i===1?-1:1,frames=[],shadows=[];
  const arcs=[[0,.25,-185,0],[.25,.43,0,30],[.43,.56,0,12],[.56,.65,0,4]];
  for(let k=0;k<=100;k++){const u=k/100;let y=0;if(u<=.25)y=-185+185*(u/.25)**2;else{const arc=arcs.slice(1).find(a=>u<=a[1]);if(arc){const f=(u-arc[0])/(arc[1]-arc[0]);y=-4*arc[3]*f*(1-f);}}
   const travel=u<.65?1-u/.65:0,spin=u<.25?u/.25*.32:.32+.68*(1-Math.pow(1-(u-.25)/.75,2));
   ...
   shadows.push({offset:u,opacity:.08+.32*(1-Math.min(1,Math.abs(y)/185)),transform:'translateX('+(-dir*24*travel)+'px) scale('+(1-.5*Math.min(1,Math.abs(y)/185))+')'});
  }
  cgAnimations.push(cube.animate(frames,{duration:1350,delay:i*85,fill:'both',easing:'linear'}));cgAnimations.push(die.querySelector('.cg-shadow').animate(shadows,{duration:1350,delay:i*85,fill:'both',easing:'linear'}));
 });}

 cgRevealTimer=setTimeout(finish,reduced?0:1550);
}
