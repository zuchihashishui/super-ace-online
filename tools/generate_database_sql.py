...
latest=int(migrations[-1].name.split('__')[0][1:])
header=f"""-- Super Ace database schema V{latest} (MySQL 8+)
-- FRESH INSTALL ONLY. Do not import this file into an existing populated database.
-- For existing V4–V{latest-1} databases use the matching upgrade file, OR simply start the new server.
...
for start in range(4,latest):
    selected=[p for p in migrations if int(p.name.split('__')[0][1:])>start]
    (directory/f'upgrade_v{start}_to_v{latest}.sql').write_text(f'-- ONLY for schema V{start}. Stop the server before importing. Back up the database first.\n-- Do not run if Flyway already applied these versions.\nUSE ace;\n'+''.join('\n-- '+p.name+'\n'+p.read_text()+'\n'+history(p) for p in selected))
print(f'Generated full MySQL install and V4–V{latest-1} upgrades at schema V{latest}.')
