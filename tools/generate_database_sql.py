...
header="""-- Super Ace database schema V8 (MySQL 8+)
...
for start in (4,5,6,7):
    ...
    (directory/f'upgrade_v{start}_to_v8.sql').write_text(f'-- ONLY for schema V{start}. Stop the server before importing. Back up the database first.\n-- Do not run if Flyway already applied these versions.\nUSE ace;\n'+''.join('\n-- '+p.name+'\n'+p.read_text()+'\n'+history(p) for p in selected))
print('Generated full MySQL install and V4/V5/V6/V7 upgrades at schema V8.')
